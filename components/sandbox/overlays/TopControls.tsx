  ...
  return (
    <div className="pointer-events-none absolute left-4 top-4 z-20 flex flex-col gap-3">
      <div className="pointer-events-auto flex items-center gap-2 rounded-xl border border-slate-800/80 bg-slate-950/40 p-2 backdrop-blur">
        <IconButton title={isRunning ? "Pause" : "Play"} onClick={() => setIsRunning(!isRunning)} active={isRunning}>
          {isRunning ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
        </IconButton>
        <IconButton title="Step forward" onClick={requestStep}>
          <StepForward className="h-4 w-4" />
        </IconButton>
        <IconButton title="Reset" onClick={requestReset}>
      ...
      <div className="pointer-events-auto flex items-center gap-2 rounded-xl border border-slate-800/80 bg-slate-950/40 p-2 backdrop-blur">
        <Slider label="Time Scale" value={timeScale} min={0} max={2} step={0.01} onChange={setTimeScale} />
        <div className="mx-2 h-8 w-px bg-slate-800" />
        ...
        <div className="text-[11px] text-slate-500">0 = freeze, 1 = realtime</div>
      </div>
    </div>
  );
}

...
export function FbdPanel() {
  const { t } = useI18n();
  const { showFbd, setShowFbd, fbdAxesMode, setFbdAxesMode, selected, fbdReadout } = useSandbox();
  ...
  return (
    <div className="pointer-events-none absolute right-4 top-4 z-30 w-[460px] max-w-[calc(100vw-2rem)]">
      <div className="pointer-events-auto overflow-hidden rounded-2xl border border-slate-800/70 bg-slate-950/40 shadow-xl backdrop-blur">
        ...
        <div className="px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-slate-400">{t("fbd.axes")}</div>
            <select
              value={fbdAxesMode}
          ...
          {!readout ? (
            <div className="mt-3 grid place-items-center rounded-xl border border-slate-800/60 bg-slate-950/30 p-6 text-xs text-slate-400">
              {t("fbd.empty")}
            </div>
          ) : (
